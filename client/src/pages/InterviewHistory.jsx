...
function InterviewHistory() {
    ...
    useEffect(() => {
        const getMyInterviews = async () => {
            try {
                const res = await axios(ServerURL + '/api/interview/get-interview', { withCredentials: true });
                        ...
                        (<div
                            ...
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    {/* Status Badge */}
                                    <span className={`px-4 py-1 rounded-full text-xs font-medium ${item.status === 'Completed' ? 'bg-green-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                        {item.status}
                                    </span>
                                </div>
